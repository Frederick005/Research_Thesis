...
def process_data(flag_plt):
    ...
    # The list contains many lists of data which are actually the per frame data. 
    # So, I am eliminating that and just taking the data which matters. 
    # In this case it is the longest list of relevant points.
    # for item in point_x:
    #     if len(item)>max:
    #         max_point_x = item
    #         max_point_y = point_y[count]
    #         max_point_vel = point_vel[count]
    #         max = len(item)
    #     count += 1
    ########################################################################################
    # Largest list among lists have been extracted
    # Using the frame with the most number of detections to perform DBSCAN clustering
        
    # Adding a section of code which checks the diversity of object ids in the list
    ...
    #default_plot(flag_plt, max_count)
    #default_plot_uf(max_uf_point_x, max_uf_point_y, max_uf_point_range)
...
def default_plot(flag_plt, max_count):
    if flag_plt == 1: 
...
######################################################################################################################################################################
# LOOOKKK HERERERER DELETE THIS FUNCTION!!!!!!!!!!
# The below 2 functions are to calculate the maximum fov of the radar sensor. 
# This is accomplished by taking the frame with the most number of points and 
# then getiing the max and min points for the x and y axis.
#
# Function to filter out the moving points from stationary points 
def Get_the_fov(frame, ego_velocity):
    interm_point_x = []
    interm_point_y = []
    interm_obj_id = []
    interm_point_vel = []
    for point in frame:
        ...
        alt = point.altitude
        target_speed= radial_vel * math.cos(azi)
        interm_point_x.append(math.degrees(azi))
        interm_point_y.append(math.degrees(alt))
        interm_obj_id.append(point.obj_id)
        interm_point_vel.append(abs(target_speed))
        # if point.obj_id > 0:
        #     interm_point_x.append(math.degrees(azi))
        #     interm_point_y.append(math.degrees(alt))
        #     interm_obj_id.append(point.obj_id)
    ...
    point_y.append(interm_point_y)
    point_obj_id.append(interm_obj_id)
    point_vel.append(interm_point_vel)
...
def get_fov():
    ...
    for item in point_x:
        ...
        count += 1
    print(maxi)
    print("Print min value of x so that we can get the range of the graph", max(max_point_x))
    print("Print min value of x so that we can get the range of the graph", min(max_point_x))
    print("Print max value of y so that we can get the range of the graph", max(max_point_y))
    print("Print min value of y so that we can get the range of the graph", min(max_point_y))
########################################################################################################################################################################   
...
def dummy_scenario_5_perpendicular_traffic(world,actorlist):
    # Creating 2 other cars in front of the ego vehicle so that the radar can be tested
    ###################################################################################
    ...
    blueprintlibrary = world.get_blueprint_library()
    vehicle_target_1_bp = blueprintlibrary.filter("etron")[0]
    transform_target_1 = carla.Transform(carla.Location(x=7, y=185, z=4), carla.Rotation(yaw=90))
    vehicle_target_1 = world.try_spawn_actor(vehicle_target_1_bp, transform_target_1)
    actorlist.append(vehicle_target_1)
    ...
    # pedestrian_bp = blueprintlibrary.filter("walker.pedestrian.0002")[0]
    # pedestrian_bp.set_attribute('speed', '10')
    # transform_target_p = carla.Transform(carla.Location(x=4, y=185, z=4), carla.Rotation(yaw=90))
    # pedestrian_target = world.try_spawn_actor(pedestrian_bp, transform_target_p)
    # #vehicle_target_2.set_autopilot(True)
    # actorlist.append(pedestrian_target)
    ...
    ##########################
    vehicle_bp = blueprintlibrary.filter('cybertruck')[0]
    transform = carla.Transform(carla.Location(x=35, y=200, z=4), carla.Rotation(yaw=180))
    vehicle = world.try_spawn_actor(vehicle_bp, transform)
    #vehicle.set_autopilot(True)
    actorlist.append(vehicle)

    vehicle_target_1.apply_control(carla.VehicleControl(throttle = 0.6, brake =0))
    #pedestrian_target.apply_control(carla.WalkerControl(direction=carla.Vector3D(0.0,1.0,0.0), speed = 2))
    vehicle.apply_control(carla.VehicleControl(throttle = 0.3, brake =0))
...
def main():
    ...
    try:
        ...
        #dummy_scenario_5_perpendicular_traffic(world,actorlist)
